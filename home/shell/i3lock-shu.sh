#!/usr/bin/env bash
# dependencies: gnugrep, procps, imagemagick, xrandr, i3lock-color, onboard
...
convert -size $screenarea xc:black -quality 11 png24:"$picresizedhint"
...
do
    ...
    convert "$picresizedhint" \
            \( "$pic" -gravity Center -resize ${screen%%+*}^ -extent ${screen%%+*} \
            -font DejaVu-Sans -fill white -stroke black -pointsize 96 -strokewidth 2 \
            -draw "text 5,5 'Locking... Press [q]/[x] to relieve.'" \) \
            -gravity NorthWest -geometry +${screen#*+} -composite \
            -quality 11 png24:"$picresizedhint"
done

feh --fullscreen --auto-zoom "$picresizedhint" &
pidfeh=$!
sleep 2
if [[ -z $(ps -ef | awk '{print $2}' | grep $pidfeh) ]]
then
    exit
else
    kill $pidfeh
...
i3lock-color -i "$picresized"\
             ...
             --ind-pos="x+w-228:y+h-32-140"  --time-pos="x+w-24:y+h-24-140" --time-align=2\
